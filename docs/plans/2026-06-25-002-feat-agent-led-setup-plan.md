...
- R4. The Telethon gateway uses profile-local credentials and session storage for login, chat resolution, history ingestion, and confirmed reply posting.
- R5. Login cancellation, missing credentials, invalid credentials, and Telegram API errors leave the profile in an actionable non-ready state.
- R6. Chat resolution stores the configured Telegram entity metadata before sync-dependent workflows proceed.

**Agent-led setup**

- R7. The Codex skill checks profile status before analytics, search, draft, or post workflows.
- R8. When a profile is not ready, the skill guides the operator through exactly the missing next step instead of attempting the normal support workflow.
- R9. The guided path runs setup, credential configuration, login, sync, crawl, index, and status checks in the order needed to reach a Ready Profile.

**Safety and documentation**

- R10. Existing confirmation-gated posting behavior remains unchanged.
- R11. Setup docs explain how to obtain Telegram API credentials, where profile-local credentials and sessions live, and how reset removes sensitive local state.
- R12. Tests cover the profile status machine, credential storage behavior, Telethon gateway fakes, and skill workflow instructions.
...
flowchart TB
  ...
  MissingConfig -->|no| Setup[Collect chat and seed, run setup]
  ...
  Corpus -->|yes| Normal[Proceed with search, stats, draft, confirm workflows]
  Build --> Status
```
...
- **Requirements:** R1, R2, R3, R12; supports origin R1-R9 and AE1-AE3.
- **Dependencies:** None.
...
- **Test scenarios:**
  ...
  - Covers AE3. Given credentials are absent, login preflight reports missing credentials instead of raising the current generic unsupported gateway error.
...
### U2. Real Telethon gateway behind the existing protocol
...
- **Requirements:** R4, R5, R6, R10, R12; supports origin R9-R10 and AE4.
- **Dependencies:** U1.
...
- **Test scenarios:**
  - Covers AE4. Given a fake authorized Telethon client, login returns success and status can treat the session as usable.
...
### U3. Corpus build readiness and status integration

- **Goal:** Make sync, crawl, index, and stale-index state visible through the profile status contract.
- **Requirements:** R1, R5, R6, R9, R12; supports origin R11-R13 and AE5.
- **Dependencies:** U1, U2.
...
### U4. Agent-surface setup guidance
...
- **Requirements:** R7, R8, R9, R10, R12; supports origin F1-F3 and AE1-AE6.
...
- **Test scenarios:**
  ...
  - Covers AE3. Skill instructions tell the agent to explain Telegram credential setup when status reports missing credentials.
  - Covers AE5. Skill instructions order sync, crawl, and index before reporting ready.
  - Covers AE6. Skill instructions preserve the evidence and confirmation path once ready.
- **Verification:** A reader can follow the skill instructions without inventing commands or bypassing the CLI status contract.

### U5. Documentation and reset behavior

- **Goal:** Document first-run setup, Telegram API credential acquisition, profile-local sensitive files, and reset behavior.
- **Requirements:** R2, R3, R8, R11; supports origin success criteria and scope boundaries.
...
- **Test scenarios:**
  ...
  - Reset instructions cover credentials and session files as well as SQLite/index data.
- **Verification:** A new operator can identify where to get Telegram API credentials, where they are stored locally, and how to reset the profile.
...
- AE3. Given missing Telegram credentials, when login is requested, then the CLI reports missing credentials and the skill explains how to provide profile-local API ID and hash.
- AE4. Given valid profile-local credentials and successful Telegram authorization, when login completes, then subsequent status sees a usable session.
- AE5. Given a logged-in profile with no corpus, when setup guidance continues, then sync, crawl, and index run before the skill reports ready.
- AE6. Given a Ready Profile, when the operator asks for a draft reply, then the skill follows the existing evidence and confirmation workflow.
...
- Update `docs/setup.md` with Telegram API credential acquisition steps, profile-local storage notes, and reset semantics.
