...
Replace the previous hash-based, in-memory retrieval projections with a SQLite-native hybrid search layer. The new index uses a durable document projection, SQLite FTS5 for BM25-style exact-term retrieval, sqlite-vec for local vector search, and `BAAI/bge-small-en-v1.5` as the committed local Hugging Face embedding model.

---

## Problem Frame

The Local Core already treats retrieval indexes as rebuildable projections over source-linked Telegram, web, and Manual Knowledge Note chunks. That shape is right, but the previous retrieval implementation was a prototype: lexical search used token-overlap scoring in Python, vector search used hash embeddings, and stored lexical/vector references were JSON side tables rather than queryable search indexes.
...
- **`BAAI/bge-small-en-v1.5` is the committed embedding model.** The model should run locally through a Hugging Face-compatible path and its identity should be recorded in index metadata.
...
- R11. Embeddings must be generated locally with `BAAI/bge-small-en-v1.5`.
...
- F1. Build a fresh SQLite hybrid index
  ...
  - **Steps:** The Local Core refreshes indexed documents, updates FTS rows, embeds documents with `BAAI/bge-small-en-v1.5`, writes sqlite-vec rows, and records a successful index run.
...
- Replacing the previous hash embedding and in-memory scoring path.
- Using SQLite FTS5 for BM25-style exact-term retrieval.
- Using sqlite-vec for local vector retrieval.
- Making `BAAI/bge-small-en-v1.5` the committed embedding model for this project.
...
- Multilingual search-quality tuning beyond what `BAAI/bge-small-en-v1.5` provides by default.
...
- `BAAI/bge-small-en-v1.5` has acceptable local runtime cost for the first product version.
...
- The previous prototype used hash embeddings and in-memory vector scoring.
